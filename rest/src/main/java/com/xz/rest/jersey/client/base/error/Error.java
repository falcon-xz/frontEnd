package com.xz.rest.jersey.client.base.error;

import com.xz.rest.jersey.client.base.Base;
import com.xz.rest.jersey.client.base.put.PutStudent;

import javax.ws.rs.client.Entity;
import javax.ws.rs.client.Invocation;
import javax.ws.rs.core.MediaType;
import javax.ws.rs.core.Response;

/**
 * Created by Administrator on 2017-9-20.
 */
public class Error extends Base {
    @Override
    public String setUrl() {
        return "/student/put";
    }

    @Override
    protected Response buildHTTP(Invocation.Builder builder) {
        Entity<String> entity = Entity.entity("name=&age=21", MediaType.APPLICATION_FORM_URLENCODED_TYPE) ;
        return builder.put(entity);
    }

    @Override
    protected void deal(Response response) {
        int i = response.readEntity(int.class);
        System.out.println(i);
    }

    public static void main(String[] args) {
        Base base = new Error() ;
        base.execute();
    }
}
